...
public class Eroder {
    ...
    public Eroder(short[][] matrizOriginal, int figura) {
    ...
    public short[][] erocionSecuencial(int startRow, int endRow) {
        ...
        switch (figura) {
            case 1: // Figura 1: min[(x,y), (x-1,y), (x+1,y), (x,y-1), (x,y+1)]
                for (int i = startRow; i <= endRow; i++) {
                    for (int j = 1; j <= ancho; j++) {
                        matrizDilatada[i - 1][j - 1] =(short) Math.min(
                            matrizExpandida[i][j],
                            Math.min(
